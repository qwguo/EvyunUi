...
+function ($) {
  ...
  Fold.TRANSITION_DURATION = 150;

  Fold.prototype.show = function () {
    var $this    = this.element,
  ...
  function Plugin(option) {
    return this.each(function () {
      var $this = $(this),
          data  = $this.data('bs.fold');

      if (!data){
        $this.data('bs.fold', (data = new Fold(this)))
      }
  ...
  $(document).on('click.bs.fold.data-api', '[data-toggle="fold"]', clickHandler)
}(jQuery);
...
+function($) {
  ...
  Page.prototype.view = function() {
    ...
    view.push('<small class="page-sum">共<em>' + conf.pageSum + '</em>页</small>');
    return '<div class="inner page-' + (conf.skin ? (function(skin) {
      ...
      return conf.skip ?
        '<div class="page-form">到第<input class="page-num-input" type="number" min="1" max="' + conf.pageSum + '" value="' + conf.curPage + '">页<em class="page-enter-btn"><s>确定</s></em></div>' : '';
    }() + '</div>';
  };
